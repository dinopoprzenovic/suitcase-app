...
> = {
  // semantic types used by the data layer
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="absolute right-0 top-11 w-[340px] bg-white border border-[#E5E5E5] rounded-xl shadow-lg overflow-hidden z-50"
          >
            {/* Header */}
            <div className="flex items-center justify-between px-4 py-3 border-b border-[#F5F5F5]">
              <div className="flex items-center gap-2">
                <span className="text-sm font-semibold text-[#0A0A0A]">Notifications</span>
                {unreadCount > 0 && (
              ...
              <div className="flex items-center gap-2">
                {unreadCount > 0 && (
                  <button
                    onClick={markAllRead}
                    className="text-[11px] text-[#737373] hover:text-[#E8643A] transition-colors duration-150"
                  >
                    Mark all read
                  </button>
                )}
            ...
            {/* List */}
            <ul className="divide-y divide-[#F5F5F5] max-h-[360px] overflow-y-auto">
              {items.length === 0 ? (
                <li className="px-4 py-8 text-center text-sm text-[#737373]">
                  No notifications
                </li>
              ) : (
                items.map((n) => {
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-start justify-between gap-2">
                          ...
                          <div className="flex items-center gap-1.5 shrink-0">
                            <span className="text-[10px] text-[#737373] whitespace-nowrap">{n.time}</span>
                            {!n.read && (
                              <span className="w-1.5 h-1.5 rounded-full bg-[#E8643A] shrink-0" />
                            )}
                          </div>
                        </div>
                        <p className="text-xs text-[#737373] mt-0.5 leading-relaxed">{n.message}</p>
                      </div>
                    </li>
                  );
                })
